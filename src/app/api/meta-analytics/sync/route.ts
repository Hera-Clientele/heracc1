...
import { syncFromGoogleSheets } from '../../../lib/metaAnalytics';
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Meta Analytics sync API: Starting sync with', sheetData.length, 'rows');
    ...
    console.error('Meta Analytics sync API error:', error);
...
export async function GET(req: NextRequest) {
  try {
    ...
    return Response.json({
      ...
      clientId,
      platform,
      message: 'Meta Analytics sync endpoint is ready'
    });
  } catch (error: any) {
    return Response.json({ error: error.message }, { status: 500 });
  }
