...
export default function TikTokWeeklyStats({ data }: { data: TikTokRow[] }) {
  const [expanded, setExpanded] = useState(false);

  // Group data by week number
  const weeks: Record<string, TikTokRow[]> = {};
  ...
  const weeklyStats = Object.entries(weeks).map(([week, rows]) => {
    ...
    const engagement = totalViews === 0 ? 0 : ((totalLikes + totalComments + totalShares) / totalViews) * 100;
